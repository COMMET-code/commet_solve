...
void project_el_values_to_nodes(Triangulation<dim, dim> &tria) {
  ...
  const hp::MappingCollection<dim> mapping(MappingFE<dim>(FE_SimplexP<dim>(1)),
                                           MappingFE<dim>(FE_Q<dim>(1)));
  // const hp::FECollection<dim> fe(FESystem<dim, dim>(FE_SimplexP<dim>(order),
  // dim), 							   FESystem<dim, dim>(FE_Q<dim>(order), dim));
  const hp::FECollection<dim> fe(FE_DGP<dim>(1), FE_DGQ<dim>(1));
  const hp::QCollection<dim> quadrature_formula(QGaussSimplex<dim>(order + 1),
                                                QGauss<dim>(order + 1));
  ...
  VectorTools::project(
      MappingFE<dim>(FE_DGP<dim>(1)), df, constraints,
...
unsigned int cell_to_n_face_verts(const dealii::CellData<dim> &cell) {
  ...
  default:
    throw std::logic_error(
        "Cannot determine number of vertices for face on cell that has " +
...
template <int dim>
vector<unsigned int>
cell_and_face_to_local_vertices(const dealii::CellData<dim> &cell,
...
template <int dim, typename tri_type> void field_test(tri_type *tri) {
  ...
  // hp::MappingCollection<dim> mapping(MappingFE<dim>(FE_SimplexP<dim>(order)),
  ...
  // const hp::FECollection<dim> fe(FESystem<dim, dim>(FE_SimplexP<dim>(order),
  // dim), FESystem<dim, dim>(FE_Q<dim>(order), dim));
  hp::QCollection<dim> quadrature_formula(QGaussSimplex<dim>(order + 1),
                                          QGauss<dim>(order + 1));
  ...
  data_out.add_data_vector(v_field.df, v_field.fields["fibre"], solution_names,
                           interpretation);
  AnalyticalVectorField<dim, double> an_field("y", "x", "0", tri, mapping,
                                              quadrature_formula);
  ...
  data_out.write_vtu_with_pvtu_record("./", base_vtu_name, 0, MPI_COMM_WORLD,
                                      n_digits);
}
...
void triangulation_from_msh(Triangulation<dim, dim> &tria,
  ...
  // std::istream stream(path_to_file);
...
void triangulation_from_json(Triangulation<3, 3> &tria,
  ...
  unsigned int b_id = 1;
  for (auto bound_it = inp["boundaries"].begin();
       bound_it != inp["boundaries"].end(); ++bound_it) {
    ...
    for (auto it = bound.begin(); it != bound.end(); ++it) {
      ...
      const vector<unsigned int> &el_verts =
          cell_and_face_to_local_vertices(cell, local_face_number);
...
void json_to_deal(Triangulation<3, 3> &tria, const std::string &path_to_file) {
  ...
  for (auto it = inp["elements"].begin(); it != inp["elements"].end(); ++it) {
    ...
    connectivity.at(key_i) =
        std::array<int, 4>({{vals[0], vals[1], vals[2], vals[3]}});
  }
